 ...
 * */
    ...
    int invoke_via_socket(std::string func_name, char *request_payload, int payload_length){
        // for response
        char buf[1024];
        int nrecved;
...
//    std::string gateway_addr, gateway_port;
    std::string gateway_addr = utils::GetEnvVariable("LUMINE_GATEWAY_ADDR", "192.168.1.129");
    std::string gateway_port = utils::GetEnvVariable("LUMINE_GATEWAY_PORT", "8084");
//
    auto request = utils::Socket(gateway_addr.c_str(), std::stoi(gateway_port));
//        auto request = utils::Socket("192.168.1.199", 8084);
        ...
        std::string invoke_path = "/function/" + func_name;

        request.issue_http_request("POST", invoke_path.c_str(), request_payload, payload_length);
